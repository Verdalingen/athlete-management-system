...
# ---------------------------------------------------------------------------
# Tier-2: lightweight weekly re-plan
# ---------------------------------------------------------------------------

def _format_recent_activities(garmin_data: dict[str, Any], days: int = 14) -> str:
    """Format recent Garmin activities as plain text for the weekly planner context."""
    activities = garmin_data.get("recent_activities") or garmin_data.get("all_activities") or []
    cutoff = (datetime.now() - timedelta(days=days)).date()

    recent: list[tuple] = []
    for act in activities:
        raw_ts = act.get("start_time") or ""
        try:
            act_date = datetime.fromisoformat(str(raw_ts)[:19]).date()
        except (ValueError, TypeError):
            continue
        if act_date >= cutoff:
            recent.append((act_date, act))
    ...
    lines = [
        f"RECENT TRAINING (last {days} days — completed sessions; use as ground truth "
        "for what was actually done vs missed before generating the new 4-week plan):"
    ]
    for act_date, act in recent:
        name = act.get("activity_name") or act.get("activity_type") or "Unknown"
        summary = act.get("summary") or {}
        dur_secs = summary.get("duration") or 0
        dist_m = summary.get("distance") or 0
        avg_hr = summary.get("average_hr")

        detail = name
        if dur_secs:
            detail += f" {int(dur_secs / 60)}min"
        if dist_m:
            detail += f" {round(dist_m / 1000, 1)}km"
        if avg_hr:
            detail += f" avg HR {avg_hr}bpm"

        lines.append(f"  • {act_date.strftime('%a %b %-d')}: {detail}")
...
async def run_replan(
    ...
    """
    activity_summary = _format_recent_activities(garmin_data, days=14)
    combined_context = f"{planning_context.rstrip()}\n\n{activity_summary}"
