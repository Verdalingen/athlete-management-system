...
STRENGTH_ACTIVITY_TYPES = frozenset({"strength_training", "fitness_equipment"})
...
def _pick_exercise(exercises: list[Any]) -> tuple[str | None, str | None]:
    """Return (category, name) for the most probable non-UNKNOWN exercise candidate."""
    candidates = [
        e for e in exercises
        if isinstance(e, Mapping) and e.get("category") != "UNKNOWN"
    ]
    if not candidates:
        return None, None
    # Sort by probability descending; prefer named variants on ties
    candidates.sort(key=lambda e: (-(e.get("probability") or 0), e.get("name") is None))
    best = candidates[0]
...
class GarminDataExtractor(DataExtractor):
    ...
    def get_exercise_sets(self, activity_id: int) -> list[ExerciseSet]:
        raw = self._call_api(
        ...
        for s in raw.get("exerciseSets") or []:
            if not isinstance(s, Mapping):
                continue
            category, name = _pick_exercise(s.get("exercises") or [])
            weight_grams = _to_float(s.get("weight"))
            weight_kg = _round(weight_grams / 1000, 3) if weight_grams is not None else None
            result.append(ExerciseSet(
                ...
                weight_kg=weight_kg,
                duration_seconds=_to_float(s.get("duration")),
    ...
    def _process_single_sport_activity(self, detailed_activity: MutableMapping[str, Any]) -> Activity | None:
        try:
            ...
            if activity_type in STRENGTH_ACTIVITY_TYPES:
                exercise_sets_out = self.get_exercise_sets(activity_id)
                laps_out = []  # lap data is meaningless for strength sessions
