...
## Progress page — gradient-fill area charts (the last redesign-plan item)

The one deliberately-deferred piece from the redesign completion pass:
threading `<linearGradient>` fade fills through the Progress page's charts,
matching the technique already used by the dashboard's `FitnessTrendChart`
(flat low-opacity fills → gradient-fade-to-transparent).

- **`CompactChart`** (the per-metric sparkline cards — ACWR, Recovery,
  VO2max, HRV, Sleep Score, Sleep Hours, etc.): each gets its own
  `<linearGradient>` with an id derived from `series.label`
  (`cc-grad-${label}`, non-alphanumeric chars stripped) — needed because
  every metric card renders its own `<svg>` with its own fill color, unlike
  `FitnessTrendChart` which only ever needs two fixed ids (`ctl-grad`/
  `atl-grad`) for one chart. Top-stop opacity 0.32, fading to 0.
- **`ExpandedChart`** (the full-width detail view opened by clicking a
  card): same per-series gradient id pattern (`ec-grad-${label}`), guarded
  behind `!isBar` since bar-chart metrics don't render an area fill at all.
  Top-stop opacity 0.22 (a larger chart reads better with a slightly
  subtler fill than the small sparklines).
- **The top "Fitness & Fatigue" CTL/ATL chart had no area fill at all**
  before this pass — just bare line strokes. Added `ctlArea`/`atlArea` path
  construction (mirrors the existing `ctlLine`/`atlLine` construction,
  closing down to the plot's baseline) and two fixed-id gradients
  (`pmc-ctl-grad`/`pmc-atl-grad`, since this chart isn't per-metric-dynamic
  like the two above) using the *exact* same stop colors/opacities as
  `FitnessTrendChart` (`--accent` @ .26→0, `--red` @ .20→0) — this Progress-
  page chart and the dashboard hero chart show the same two metrics, so
  they should look like the same chart language, not two different ones.
- Verified live: sparkline cards, the expanded ACWR detail view (fill
  correctly layers under the zone-colored line segments and dashed
  threshold markers), and the top Performance Management chart all show
  the fade now. No visual regressions on the bar-chart metrics (ramp rate,
  TSB), which were correctly left untouched since bars don't have an area
  fill concept.

This closes out every item from the original frontend-redesign plan.
